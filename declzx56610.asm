...
TYPZX7:	equ	3	;; Original ZX7 compression (similar to known ZX7 compressor)
TYPBLK:	equ	4	;; Bitstream BLK (elias-gama length for both unpack/packed blocks)
TYPBS1:	equ	5	;; Bitstream standart BS1 (0=unp,10=2sek,110=3sek,1110+EG=sekvence,1111+EG=unp)

;; Supported offset codings

POSOF4:	equ	4	;; Four step offsets (1,2,3,4 / 2,4,6,8 / 3,6,9,12 / 4,8,12,16 bits)
POSOF1:	equ	5	;; One fixed offset (ofs1 = number of offset bits)
POSOF2:	equ	6	;; Two fixed offsets  (ofs1,ofs2 = two numbers of offset bits)
POSOFD:	equ	7	;; Simple elias-gama coding offset (ofs1,ofs2 not used)
...
com:	equ	5	;; Compression type - one from TYPZX7 TYPBLK TYPBS1
pos:	equ	6	;; Offset coding - one from POSOF4 POSOF1 POSOF2 POSOFD
ofs1:	equ	6	;; Number of bits for 1st offset - required for POSOF1 POSOF2 POSOF4
ofs2:	equ	10	;; Number of bits for 2nd offset - required for POSOF2 only
...
;; Yield to background music thread
...
declop:
	...
	pop	af
      ENDIF
	call	getlen		;; Get length of next block              *** BLK compression ***
	ret	c		;; If length > 65535 then end of data (end mark)
      IF spd < 2
	call	getbit		;; Get bit from bitstrem
      ELSE
...
;;;	ASSERT	ofs1 >= 1 and ofs1 <= 16
	...
	ld	ixl,ofs2-ofs1	;; We will read N bits, N = difference between bitwide of offsets
      IF spd < 2
	call	getbit		;; Get bit from bitstrem
      ELSE
	add	a,a		;; Get bit optimized for speed
	...
	ld	ixl,ofs1		;; Number of bits of shorter offset
...
;; Get value from bitstream
    ...
    IF com==TYPBS1 | pos==POSOF1 | pos==POSOF2 | pos==POSOF4
      IF com==TYPZX7 | com==TYPBLK | pos==POSOFD
...
getnxt:	rl	c		;;  (in case of reading elias-gama we have CY=1 for first time)
	...
	defb	$DD
	dec	l;ixl		;;  (value > 65535 is usually used as end mark)
